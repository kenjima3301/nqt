...
<!--                  <h6>Tồn kho hiện tại:{{$tyre->quantity}}</h6>
                  <form method="POST" action="{{url('dealer/xuat-lop-xe-tai')}}" class="d-flex flex-column align-items-center" enctype="multipart/form-data">
                    @csrf
              ...
              <img class="" src="{{asset($image->image)}}" alt="car image">
              @endforeach
                </div>
              </div>
            </div>
  ...
  <div class="table-responsive">
    <table class="table" style='font-weight:200; line-height:0.85'>
      ...
      <tbody>
        @foreach($tyre_dimentions as $tyre)
        <form action="{{url('dealer/cap-nhat-gia-so-luong-cho-lop')}}" method="POST">
        @csrf
        <input type="hidden" name="dimention_id" value="{{$tyre->id}}">
        <tr>
          <td class='p-0'>
            @foreach ($tyre->dimention->madeins as $country) 
            @if(count($tyre->dimention->madeins) ==1 && $country->country->name == 'Thailand')
              &nbsp;&nbsp;
            @endif
            <img src="{{asset($country->country->flag)}}" width='10'>
            @endforeach
          </td>
          <td class=" text-center text-sm p-0"> {{$tyre->dimention->size}} </td>
          <td class=" text-center text-sm p-0"> {{$tyre->dimention->ply}} </td>
          <td class=" text-center text-sm p-0"> {{$tyre->dimention->sevice_index}} </td>
          <td class=" text-center text-sm p-0"> <input type="number" name="total" value="{{$tyre->total}}"></td>
