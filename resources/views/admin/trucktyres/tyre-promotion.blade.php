        ...
        <div class="col-lg-5">
          <div class="row">
            <div class="col-lg-12 col-md-12 col-12 mt-2">
              ...
              <img class="" src="{{asset($image->image)}}" alt="car image" style="max-width: 100px;">
              @endforeach
                </div>
              </div>
            </div>
    ...
    <table class="table" style='font-weight:200; line-height:0.85'>
      <thead>
        <tr>
          ...
          <th class="text-xxs p-0 text-center">GIÁ</th>
          <th class="text-xxs p-0 text-center">GIÁ KHUYẾN MẠI</th>
        </tr>
      </thead>
      <tbody>
        @foreach($tyre->dimentions as $dimention)
      <form action="{{url('admin/lop-xe-tai-promotion-tyre-add')}}" method="POST">
        @csrf
        ...
        <tr>
          ...
          <td class=" text-center text-sm p-0"> 
            <input type="number" name="price" value="{{$dimention->promotion->promotion_price ?? ''}}"> </td>
          <td><input type="submit" value="Cập nhật khuyến mãi"></td>
        </tr>
      </form>
        @endforeach
