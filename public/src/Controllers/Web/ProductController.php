...
class ProductController extends Controller
{
    ...
    public function cat($slug, Request $request)
    {
        ...
        $seoPage['type'] = $this->infoSeo('product', $this->type, 'type', 'index');
        Seo::setSeoData($seoPage, 'product', 'seo');
        return View::share(['itemCat' => $itemList['id'], 'com' => $this->type])->view('product.product', compact('product', 'titleMain', 'listProperties'));
    }
    public function item($slug, Request $request)
    {
        ...
        $seoPage['type'] = $this->infoSeo('product', $this->type, 'type', 'index');
        Seo::setSeoData($seoPage, 'product', 'seo');
        return View::share(['itemItem' => $itemList['id'], 'com' => $this->type])->view('product.product', compact('product', 'titleMain', 'listProperties'));
    }
    ...
    public function filterProduct(Request $request){
        $product = ProductTagsModel::select('id','id_parent');
        if ($request->query('brand')) {
            $product->orWhere('id_tags', $request->query('brand'));
        } 
        if ($request->query('size')) {
            $product->orWhere('id_tags', $request->query('size'));
        }
        if ($request->query('code')) {
            $product->orWhere('id_tags', $request->query('code'));
        }
        if ($request->query('type')) {
            $product->orWhere('id_tags', $request->query('type'));
        }
        $product=$product->orderBy('id', 'desc')->get();
        $product_id=[];
        foreach ($product as $key => $v) {
            $product_id[$key]=$v['id_parent'];
        }
        $product=ProductModel::select('*')
            ...
            ->paginate(12);
        $titleMain='Tìm kiếm sản phẩm';
        return View::share('com', 'san-pham')->view('product.product', compact('product','titleMain'));
    }
