...
class ProductController
{
    ...
    public function save($com, $act, $type, Request $request)
    {
        if (!empty($request->csrf_token)) {
            ...
            $dataBrand = (!empty($request->input('dataBrand'))) ? $request->input('dataBrand') : null;
            $dataSize = (!empty($request->input('dataSize'))) ? $request->input('dataSize') : null;
            $dataCode = (!empty($request->input('dataCode'))) ? $request->input('dataCode') : null;
            $dataType = (!empty($request->input('dataType'))) ? $request->input('dataType') : null;
            ...
            if ($id) {
                ...
                if (ProductModel::where('id', $id)->where('type', $type)->update($data)) {
                    if (!empty($this->configType->$type->tags)) {
                        ...
                        $this->insertTags(ProductTagsModel::class, $request, $dataBrand, $id, $type.'-brand');
                        $this->insertTags(ProductTagsModel::class, $request, $dataSize, $id, $type.'-size');
                        $this->insertTags(ProductTagsModel::class, $request, $dataCode, $id, $type.'-code');
                        $this->insertTags(ProductTagsModel::class, $request, $dataType, $id, $type.'-type');
                    }
