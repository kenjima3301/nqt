...
@if (!empty($destop) || !empty($mobile))
    ...
    ">
        <ul>
            @foreach ($mxh as $mxh)
                <li>
                    <a class=" text-decoration-none " href="{{ $mxh['link'] }}" title="{{ $mxh['name'.$lang] }}">
                        <img data-src="{{ upload('photo', $mxh['photo']) }}" alt="{{ $mxh['name'.$lang] }}" class="lazy loaded"
                            src="{{ upload('photo', $mxh['photo']) }}" data-was-processed="true"><br>
                        <span>{{ $mxh['name'.$lang] }}</span>
                    </a>
                </li>
            @endforeach
        </ul>
