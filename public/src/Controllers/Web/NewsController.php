...
class NewsController extends Controller
{
    ...
    public function detail($slug)
    {
        ...
        return View::share('com', $rowDetail->type)->view($template, compact('rowDetail', 'rowDetailPhoto', 'news', 'tags'));
    }
    public function newsAlbum(){
        $news =NewsModel::select('id', 'namevi', 'nameen', 'descvi', 'descen', 'slugvi', 'slugen', 'photo')
            ->where('type', $this->type)
            ->whereRaw("FIND_IN_SET(?,status)", ['hienthi'])
            ->datePublish()
            ...
            ->orderBy('id', 'desc')
            ->paginate(12);
        $titleMain = config("type.news.$this->type.title_main");
        BreadCrumbs::setBreadcrumb(type: $this->type, title: $titleMain);
        $this->seoPage($titleMain,$this->infoSeo('news', $this->type, 'type','index'));
        return View::share('com', $this->type)->view('album.album', compact('news', 'titleMain'));
    }
}